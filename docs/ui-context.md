...
## 16. Theme Rule
The project remains dark-only at the global theme level until a real light palette is designed. Do not add a theme toggle simply because next-themes exists. Light/ivory sections are allowed as intentional editorial sections. The dark → light change is a scroll-driven page atmosphere (see `docs/tailwind-conversion-notes.md` §13), not a theme toggle. The homepage alternates dark → light → dark across its sections; every other public route has exactly one transition: a dark hero band, then light for the content the visitor came to read (long-form copy reads better dark-on-light). A manual toggle and a time-of-day switcher were both considered and rejected by the project owner; do not add either, and do not give inner pages the homepage's multi-section alternation.
