...
- Content colour must follow the atmosphere, otherwise it is unreadable mid-transition. Use the semantic utilities: `text-atmos` (set on `<main>`), `text-atmos-muted`, `border-atmos-line`, `bg-atmos-tint`, or `currentColor`. Do not use `text-brand-black` / `text-brand-ivory` (or their opacity variants) for content that lives inside an atmosphere page; only the footer is a self-contained dark surface (`.atmos-dark-surface`). `EventCard` and `ProgramCard` follow the atmosphere too (see "Cards" below).
...
- Cards follow the atmosphere. `ProgramCard` and `EventCard` use `bg-atmos-card` (fill), `bg-atmos-card-media` (image block), `border-atmos-line`, `text-atmos` and `text-atmos-card-muted`, plus `bg-atmos text-atmos-inverse` for the date/badge pill. The fill and media tokens ease with `--atmos-t` exactly like the body background: in a dark zone the card is the navy card, in a light zone it is an outlined ivory card with a plain light-grey image block (the same treatment as `PostCard`, which never had a fill). Do not hardcode `brand-navy` / `brand-ivory` in a card: a dark box on the ivory page is a bug, not a design choice. This applies to every card on the homepage too: its Performances and Programs sections are `data-theme="light"`, so their cards are light.
